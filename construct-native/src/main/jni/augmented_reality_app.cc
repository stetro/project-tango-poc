...
namespace tango_augmented_reality {
    void AugmentedRealityApp::onPointCloudAvailable(const TangoXYZij *xyz_ij) {
    ...
    int AugmentedRealityApp::TangoSetupConfig() {
        ...
        if (ret != TANGO_SUCCESS) {
            LOGE(
                    "AugmentedRealityApp: config_enable_depth() failed with error"
                            "code: %d",
                    ret);
            return ret;
        }
    ...
    int AugmentedRealityApp::TangoConnectCallbacks() {
        ...
        if (ret != TANGO_SUCCESS) {
            LOGE(
                    "AugmentedRealityApp: Failed to connect to point cloud callback with error"
                            "code: %d",
                    ret);
            return ret;
        }
...
// pose can be queried.
    ...
    int AugmentedRealityApp::GetPointCloudVerticesCount() {
        std::lock_guard<std::mutex> lock(point_cloud_mutex_);
        return point_cloud_data_.GetPointCloudVerticesCount();
    }

    float AugmentedRealityApp::GetAverageZ() {
        std::lock_guard<std::mutex> lock(point_cloud_mutex_);
        return point_cloud_data_.GetAverageDepth();
    }

    float AugmentedRealityApp::GetDepthFrameDeltaTime() {
        std::lock_guard<std::mutex> lock(point_cloud_mutex_);
