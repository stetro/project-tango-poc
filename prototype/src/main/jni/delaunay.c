...
#include "tango-augmented-reality/delaunay.h"
...
struct working_set_s {
    halfedge_t *edges;
    /* all the edges (allocated in one shot) */
    face_t *faces;
    /* all the faces (allocated in one shot) */

    unsigned int max_edge;
    /* maximum edge count: 2 * 3 * n where n is point count */
    unsigned int max_face;
    /* maximum face count: 2 * n where n is point count */
    ...
    halfedge_t *free_edge;
    /* pointer to the first free edge */
