...
namespace {
    ...
    // handling
    //
    // @param context, context will be a pointer to a AugmentedRealityApp instance
    //        on which to call callbacks
    // @param XYZ_ij, the actual depth data in XYZij format
    ...
    // @param context, context will be a pointer to a AugmentedRealityApp instance
    //        on which to call callbacks
    // @param buffer, the actual image data as TangoImageBuffer - HAL_PIXEL_FORMAT_YV12
...
namespace tango_augmented_reality {
    ...
    int AugmentedRealityApp::TangoInitialize(JNIEnv *env, jobject caller_activity) {
        ...
        calling_activity_obj_ = reinterpret_cast<jobject>(env->NewGlobalRef(caller_activity));
    ...
    int AugmentedRealityApp::TangoSetupConfig() {
        ...
        if (ret != TANGO_SUCCESS) {
            LOGE("config_enable_low_latency_imu_integration() failed with error code: %d", ret);
            return ret;
        }
        ...
        if (ret != TANGO_SUCCESS) {
            LOGE("get tango core version failed with error code: %d", ret);
            return ret;
        }
    ...
    int AugmentedRealityApp::TangoConnect() {
        ...
        if (ret != TANGO_SUCCESS) {
            LOGE("Failed to query sensor extrinsic with error code: %d", ret);
            return ret;
        }
        return ret;
    }
    ...
    glm::mat4 AugmentedRealityApp::GetPoseMatrixAtTimestamp(double timstamp) {
        ...
        if (status != TANGO_SUCCESS) {
            LOGE("Failed to get transform between the Start of service and device frames at timstamp %lf",
                 timstamp);
        }
    ...
    TangoErrorType AugmentedRealityApp::UpdateExtrinsics() {
        ...
        if (ret != TANGO_SUCCESS) {
            LOGE("Failed to get transform between the IMU frame and device frames");
            return ret;
        }
        ...
        ret = TangoService_getPoseAtTime(0.0, frame_pair, &pose_data);
        if (ret != TANGO_SUCCESS) {
            LOGE("Failed to get transform between the color camera frame and device frames");
            return ret;
        }
